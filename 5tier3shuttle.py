...
def source(env, interval):
    ...
    while True:
        ...
        if proc_check[2] == 0:
            env.process(shuttle_action3(env, shuttle, 3))
    ...
    while True:
        ...
        if len(active_transactions) > 0 and shuttle_avail[shuttleID - 1] == 0:
            ...
            observation = np.array([shuttle_locations[shuttleID]["tier"], shuttle_locations[shuttleID]["bay"],
                                    lift1_locations_[1], avail_lift11, lift1_locations_[2], avail_lift12,
                                    queue_state[0], queue_state[1], queue_state[2], queue_state[3], queue_state[4],
                                    queue_state[5], queue_state[6], queue_state[7], queue_state[8], queue_state[9]])
            for transaction in range(len(active_transactions)):
            ...
            if available_transactions:
                ...
                for ts in range(len(available_transactions)):
                    if available_transactions[ts][1] == tmp_type and available_transactions[ts][2] == tmp_tier and \
                            available_transactions[ts][3] == tmp_bay:
                        for transaction in range(len(active_transactions)):
                        ...
                        queue_state[(tier - 1) * 2 + type] -= 1
                        if liftno == 1:
                            if type == 0:
                                observation_ = np.array([tier, bay, tier, 1, lift1_locations_[2], 1,
                                                         queue_state[0], queue_state[1], queue_state[2], queue_state[3],
                                                         queue_state[4],
                                                         queue_state[5], queue_state[6], queue_state[7], queue_state[8],
                                                         queue_state[9]])
                            else:
                                observation_ = np.array([tier, 1, 1, 1, lift1_locations_[2], 1,
                                                         queue_state[0], queue_state[1], queue_state[2], queue_state[3],
                                                         queue_state[4],
                                                         queue_state[5], queue_state[6], queue_state[7], queue_state[8],
                                                         queue_state[9]])
                            ...
                            if type == 0:
                                observation_ = np.array([tier, bay, 1, lift1_locations_[1], tier, 1,
                                                         queue_state[0], queue_state[1], queue_state[2], queue_state[3],
                                                         queue_state[4],
                                                         queue_state[5], queue_state[6], queue_state[7], queue_state[8],
                                                         queue_state[9]])
                            else:
                                observation_ = np.array([tier, 1, 1, lift1_locations_[1], 1, 1,
                                                         queue_state[0], queue_state[1], queue_state[2], queue_state[3],
                                                         queue_state[4],
                                                         queue_state[5], queue_state[6], queue_state[7], queue_state[8],
                                                         queue_state[9]])
    ...
    if type == 0:
        ...
        for i in range(len(tier_buffer)):
            ...
            if tier_check == tier and name_check == name:
                ...
                agent.remember(observation, action, reward, observation_, False)
                agent.learn()
                lift1_util[liftno - 1] = lift1_util[liftno - 1] + flow_time
...
class Agent(object):
    def __init__(self, alpha, gamma, n_actions, epsilon, batch_size, input_dims,
                 epsilon_dec=0.99, epsilon_end=0.01, mem_size=1000000, fname='dqn_model_3shuttle.h5'):
    ...
    def learn(self):
        ...
        _ = self.q_eval.fit(state, q_target, verbose=2)
...
agent = Agent(gamma=0.4, epsilon=1.0, alpha=0.01, input_dims=16, n_actions=tiers * bays * 2 * 2,
